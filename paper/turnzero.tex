...
\begin{abstract}
TurnZero is a calibrated decision-support system for competitive Pok\'emon VGC, trained on 212K best-of-three tournament replays (382K directed examples, one per player per game).
Given two Open Team Sheets (12 Pok\'emon, full movesets), it produces a probability distribution over all 90 legal team-preview actions---which 4 to bring and which 2 to lead.
Experts routinely disagree on the correct play for the same matchup, so I treat uncertainty quantification as the primary objective rather than point accuracy.
The core result: a 5-member deep ensemble of permutation-invariant transformers needs only 17 predictions to cover 50\% of expert actions in a 90-way action space.
Its top-3 lead-pair suggestions match the expert 43\% of the time.
The ensemble is well-calibrated (adaptive ECE $= 0.012$)---when it says 15\%, it means 15\%---and reliably detects out-of-distribution teams, doubling its abstention rate on unseen archetypes (AUROC $= 0.80$).
Per-team analysis reveals that predictability is driven by action concentration: mechanically constrained ``commander'' teams have a single dominant play (50\% top-1, matching the mode frequency), while flexible ``goodstuffs'' teams spread across many viable plans (0\% top-1).
\end{abstract}
...
\begin{itemize}
    ...
    \item A complete UQ pipeline: 5-member deep ensemble, reliability calibration (adaptive ECE $= 0.012$), risk-coverage analysis (AURC $= 0.890$, E-AURC $= 0.452$), and distribution-shift sensitivity quantified by AUROC $= 0.80$.
    \item Per-team predictability analysis showing that aggregate accuracy masks a heterogeneous distribution where per-team accuracy tracks action-mode frequency ($r = 0.55$), driven by mechanical constraints rather than opponent modeling.
...
\textbf{Dataset.}
I parse 212K Regulation~G battles from Pok\'emon Showdown into 425K directed examples (one from each player's perspective), then deduplicate to 382K, sourced from the \texttt{vgc-battle-logs} dataset~\cite{vgcbench}.
Data is drawn exclusively from best-of-three tournament play---the official VGC competition format---rather than ladder games, so that training conditions match deployment.
...
Each model converges in ${\sim}$8 epochs (${\sim}$5 minutes; patience-15 early stopping runs ${\sim}$21--26 total).
...
\begin{table}[t]
\centering
\caption{Test set performance (Regime A, Tier 1, $n{=}$32,328). Top-1/3 accuracy (\%, $\uparrow$), NLL ($\downarrow$), adaptive ECE ($\downarrow$, equal-mass binning~\cite{nixon2019}). Bootstrap 95\% CIs for the ensemble are cluster-aware ($B{=}$1000). The logistic baseline's worse NLL than popularity reflects overconfident wrong predictions.}
\label{tab:main}
\small
...
NLL of 4.031 (vs.\ 4.497 popularity) confirms the model learns matchup-specific signal beyond metagame frequencies, and adaptive ECE of 0.012 means the confidence scores are honest.
...
\begin{figure}[t]
    ...
    \begin{subfigure}[t]{0.48\columnwidth}
        ...
        \label{fig:riskcov}
    \end{subfigure}
    \caption{\textbf{(a)} Predicted confidence closely tracks observed accuracy---the ensemble's probabilities mean what they say. \textbf{(b)} By abstaining on low-confidence predictions, the ensemble achieves lower error at every coverage level. AURC: 0.890 (ensemble) vs.\ 0.905 (single) vs.\ 0.953 (logistic). E-AURC: 0.452 (ensemble) vs.\ 0.404 (top-3).}
...
The ensemble's AURC of 0.890 (E-AURC $= 0.452$ after normalizing for the 93.6\% base error rate) vs.\ 0.905 single and 0.953 logistic confirms that ensembling improves not just accuracy but the model's ability to rank its own predictions by reliability.
...
Mutual-information-based ID/OOD discrimination achieves AUROC $= 0.80$, confirming that epistemic uncertainty reliably separates in-distribution from novel team compositions.
...
Calibration degrades on OOD data (adaptive ECE: $0.012 \to 0.076$).
...
\begin{figure}[t]
    ...
    \caption{Mean ensemble entropy vs.\ top-3 accuracy for 153 team archetypes ($\geq$20 Tier~1 examples). Point size $\propto n$. $r = {-}0.56$: low-entropy teams have concentrated action distributions; high-entropy teams spread across many viable plans.}
    \label{fig:entropy_scatter}
\end{figure}

The aggregate 6.4\% top-1 average obscures substantial heterogeneity.
...
the most predictable teams are ``commander'' archetypes (e.g., Dondozo + Tatsugiri, $H = 3.10$, 50\% top-1) where the Commander ability mechanically requires both Pok\'emon to lead together, concentrating the action distribution into a single dominant play.
The model correctly identifies these concentrated distributions---per-team accuracy correlates with action-mode frequency ($r = 0.55$)---but this reflects learning \emph{which teams have solved lead selection}, not opponent-conditional reasoning.
The least predictable are flexible ``goodstuffs'' teams with many viable configurations ($H = 4.41$, 0\% top-1), where the model's predictions miss the ground truth entirely.
...
\begin{table}[t]
\centering
\caption{Feature masking stress test (Regime A, Tier 1). Each row masks a cumulative set of input features and re-evaluates the frozen ensemble.}
...
Table~\ref{tab:ablation_features} shows a 7-level feature masking stress test.
Moves dominate: masking all four drops top-3 from 15.5\% to 3.2\%, near the popularity baseline's 3.9\%.
Items and tera type each cost ${\sim}$3pp individually.
Species alone still beats random (top-1: 1.2\% vs.\ 1.1\%), confirming composition-level patterns.
...
\subsection{Label Quality Ablation}

Roughly 20\% of training examples (Tier~2) have fabricated action-90 labels: the back-2 is filled with a deterministic lowest-index heuristic when fewer than 4 Pok\'emon appear in the log.
To test whether this noise matters, I compare three loss modes across 5-member ensembles (5 seeds each):
(a)~standard cross-entropy on all data (\textsc{all}),
(b)~a multi-task loss that marginalizes Tier~2 labels to lead-2 via a $90{\times}15$ aggregation matrix (\textsc{multitask}),
and (c)~training on Tier~1 only (\textsc{tier1-only}).

\begin{table}[t]
\centering
\caption{Label quality ablation (Regime A, Tier 1 test). Differences are $\leq$0.6pp top-1.}
\label{tab:ablation_label}
\small
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{Loss mode} & \textbf{Top-1} & \textbf{Top-3} & \textbf{NLL} \\
\midrule
\textsc{all} (baseline) & 6.4 & 15.5 & 4.022 \\
\textsc{multitask} & 6.5 & 15.8 & 4.017 \\
\textsc{tier1-only} & 5.9 & 15.1 & 4.047 \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:ablation_label} shows minimal differences.
Dropping Tier~2 entirely (\textsc{tier1-only}) slightly hurts performance ($-$0.5pp top-1), consistent with losing 20\% of training data.
The principled marginalization loss (\textsc{multitask}) offers no significant gain over na\"ive cross-entropy on all data---the model is robust to ${\sim}$20\% label noise in the back-2 component, unsurprising given that the noisy labels still carry correct lead-2 information.
This validates the decision to train on all data with standard cross-entropy.
...
A concrete source of this multi-modality is within-set adaptation.
Across 53K linkable BO3 sets (136K game-to-game transitions), players change their lead pair between games 59\% of the time---rising to 72\% after a loss versus 46\% after a win ($\chi^2 = 10{,}126$, $p < 10^{-6}$).
This means the training set contains genuine label contradictions: the same matchup produces different expert leads not because experts disagree on the optimal play, but because the \emph{same} player adapts to revealed information from the prior game.
No turn-zero model can capture this within-player rationality, establishing a principled floor on irreducible label noise.
...
\textbf{Limitations.}
Bootstrap CIs are wide (top-1: [2.6\%, 6.6\%]). Calibration degrades OOD (adaptive ECE: $0.012 \to 0.076$). Training data is drawn exclusively from BO3 tournament play (the official VGC competition format); ladder games were deliberately excluded to match deployment conditions.
