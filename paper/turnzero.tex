...
Per-team analysis reveals that predictability is bimodal: mechanically constrained ``commander'' teams reach 50\% top-1 accuracy, while flexible ``goodstuffs'' teams sit at 0\%.
\end{abstract}
...
The ensemble also achieves the lowest Brier score (0.974 vs.\ 0.989 popularity), confirming calibration under a proper scoring rule.
...
The correlation is strong ($r = {-}0.56$ for top-3) and domain-plausible (partly mechanical for any calibrated model, but the domain structure---Commander vs.\ goodstuffs---confirms genuine predictability differences):
